  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {unread > 0 && (
          <span className="absolute top-1 right-1 w-2 h-2 bg-indigo-500 rounded-full ring-2 ring-slate-950 animate-pulse" />
        )}
      </button>

      {open && (
        <div className="absolute right-0 top-12 w-84 rounded-2xl border border-slate-700/60 bg-slate-900/95 backdrop-blur-xl shadow-2xl shadow-black/60 z-50 overflow-hidden"
          style={{ width: "320px" }}>
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800/60">
            <div className="flex items-center gap-2">
              ...
              {unread > 0 && (
                <span className="px-1.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 text-[10px] font-bold border border-indigo-500/20">
                  {unread} new
                </span>
              )}
            </div>
            {unread > 0 && (
              <button
                ...
                className="text-[10px] text-slate-500 hover:text-indigo-400 transition font-medium"
              >
                Mark all read
              </button>
            )}
          ...
          <div className="max-h-80 overflow-y-auto">
            {items.length === 0 && (
              <div className="px-4 py-10 text-center">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-8 h-8 mx-auto mb-2 text-slate-700">
                  <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0" />
                </svg>
            ...
            {items.map((n) => {
              ...
              return (
                <div key={n.id} className={`px-4 py-3 flex items-start gap-3 border-b border-slate-800/40 last:border-0 transition-colors ${n.read ? "opacity-45" : "hover:bg-slate-800/30"}`}>
